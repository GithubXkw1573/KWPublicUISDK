...
- (void) distributeSpacingHorizontallyWith:(NSArray*)views
                              leadingSpace:(CGFloat)leadSpace
                                 tailSpace:(CGFloat)tailSpace
{
    ...
    UIView *firstV = views[0];
    [firstV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(ws.mas_left).offset(leadSpace);
    }];
    
    UIView *lastView = views[0];
    for ( int i = 1 ; i < views.count; ++i )
    {
        ...
        [obj mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(space.mas_right);
        }];
    ...
    [lastView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(ws.mas_right).offset(-tailSpace);
    }];
